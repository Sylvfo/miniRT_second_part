...
/*   Updated: 2025/05/27 15:32:54 by syl              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	prepare_v_light(t_mem *memory_shuttle, t_coord *lux_p_coord)
{
	...
	scalar_mult_na(&tmp, memory_shuttle->v_norm_parral, 0.00001);
	scalar_mult_na(&tmp, memory_shuttle->v_norm_parral, EPSILON * 10.0f);
	...
	memory_shuttle->distance_light_p_touch = length_vector(memory_shuttle->v_light_to_point);
...
bool	intersect_objects_shadow(t_scene *scene, t_mem *memory_shuttle, int lux_num)
{
	...
	in_shadow = false;
	a = 0;
	while (scene->obj[a] != NULL)
	{
		b = 0;
		while (scene->obj[a][b] != NULL)
		{
			if (a == SPHERE)
