...
/*   Updated: 2025/06/10 18:38:27 by syl              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  take_dimensions(int keycode, t_scene *scene)
{
	...
	if (keycode == 65293)
	{
	//	scene->size_modif += scene->size_modif_virg;
	...
	if (keycode >= 48 && keycode <=  57) //chiffres horiz clavier
	{
		...
		if (scene->after_virgule > 0)
			scene->size_modif += (keycode - 48)/(scene->after_virgule) * 10;
		printf("distance is %.5f \n",  scene->size_modif );
	}
	if (keycode == 46 && scene->after_virgule == 0.0)
		scene->after_virgule == 1;
}
/*
...
//	printf("axe draw mode %i \n", scene->draw_mode);
	if (scene->axe_draw != 0)
	{
		...
		//ici changer selon transformations....
		// et entrer enter
		mlx_string_put(scene->ima->mlx_ptr, scene->ima->mlx_win, (WND_WIDTH + 30), 920, 255, "CHOOSE DIMENSION / ROTATION ANGLE");
	}	
}
